...
export const PRIVACY_POLICY_SECTIONS: readonly LegalSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'Optional product usage analytics (off by default for new installs; turn on in Profile → Usage Analytics) record approximate time spent on app areas plus anonymous route visits, transitions, success or failure outcomes, and coarsened page-load or named-action durations. A server-generated anonymous device label groups these aggregate actions by app installation for up to 90 days; it does not include a device name, advertising ID, Apple or Google identifier, account ID, IP address, or the raw installation ID. Route parameters, Health notes, financial records, meal contents, messages, and other private text are never included. Only installs that turn analytics on may upload these anonymous flow aggregates. Turning analytics off stops collection and clears pending flow events.',
    ],
  },
  {
